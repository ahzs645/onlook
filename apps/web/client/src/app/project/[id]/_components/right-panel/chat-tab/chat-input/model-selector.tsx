...
function CodexLogo(props: ProviderLogoProps) {
    return (
...
function ClaudeLogo(props: ProviderLogoProps) {
    return (
        <svg {...props} preserveAspectRatio="xMidYMid" viewBox="0 0 256 257" fill="none">
            <path
                fill="currentColor"
...
function providerLogoClassName(cli: DesktopLocalChatCli, className: string) {
    return cli === 'claude' ? 'text-[#d97757]' : className;
}
...
export function DesktopLocalChatModelSelector({
    ...
    const activeProvider = lockedProvider ?? selection?.cli ?? null;
    const triggerLabel = selection
        ? getDesktopLocalChatModelLabel(selection.cli, selection.model)
        : isLoading
...
function ProviderLogo({
    ...
    const Logo = PROVIDER_LOGOS[cli];
    return (
        <Logo
            aria-hidden="true"
            className={cn('h-4 w-4 shrink-0', providerLogoClassName(cli, 'text-foreground-secondary'), className)}
        />
    );
}
